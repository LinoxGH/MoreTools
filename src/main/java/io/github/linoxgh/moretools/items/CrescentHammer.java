...
public class CrescentHammer extends SimpleSlimefunItem<ItemInteractHandler> implements DamageableItem {
    ...
    private final boolean isChestTerminalInstalled =
    ...
    public ItemInteractHandler getItemHandler() {
        return e -> {
            ...
            if (b != null) {
                ...
                if (SlimefunPlugin.getProtectionManager().hasPermission(p, b.getLocation(),
                    ProtectableAction.BREAK_BLOCK)) {

                    Long lastUse = lastUses.get(p.getUniqueId());
                    if (lastUse != null) {
                        if ((System.currentTimeMillis() - lastUse) < cooldown) {
                            p.sendMessage(Messages.CRESCENTHAMMER_COOLDOWN.getMessage().replace("{left-cooldown}",
                                String.valueOf((cooldown - (System.currentTimeMillis() - lastUse)) / 1000)));
                            return;
                        }
                    }
        ...
        if (sfItem != null) {
            if (sfItem.getID().startsWith("CARGO_NODE_")) {
                ...
                if (frequency != null) {
                    ...
                    if (current == 16) {
                        menu.replaceExistingItem(slotCurrent,
                            new CustomItem(HeadTexture.CHEST_TERMINAL.getAsItemStack(),
                                "&bChannel ID: &3" + (current + 1)));
                    } else {
                        menu.replaceExistingItem(slotCurrent,
                            new CustomItem(MaterialCollections.getAllWoolColors().get(current),
                                "&bChannel ID: &3" + (current + 1)));
                    }
                    menu.addMenuClickHandler(slotCurrent, ChestMenuUtils.getEmptyClickHandler());

                    p.sendMessage(Messages.CRESCENTHAMMER_CHANNELCHANGESUCCESS.getMessage().replace("{channel}",
